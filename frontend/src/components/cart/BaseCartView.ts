  ...
  return `
      <div class="flex gap-4 justify-start items-center border-b-2 py-3">
        ...
        <div class="flex justify-center items-center gap-1">
          <button data-prod="${prod.id}" data-qty="${prod.stock}" class="plus bg-orange-800 block w-maxselect-none text-white m-1 w-8 text-xl">+</button>
          <p class="quantity" data-id="${prod.id}">${quantity}</p>
          <button data-prod="${prod.id}" class="minus bg-orange-800 block w-maxselect-none text-white m-1 w-8 text-xl">-</button>
        </div>
